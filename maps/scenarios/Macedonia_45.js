	...
	for (let p of [8])
	{
		...
		if (soldiers.length < 300)
		{
			...
			if (patrol_sites.length >= 3)
			{
				...
				let inf_templates = ["units/pers_champion_infantry","units/pers_champion_elephant","units/pers_arstibara","units/pers_kardakes_skirmisher","units/maur_champion_maiden","units/maur_champion_maiden_archer","units/maur_elephant_archer_e"];
				...
				//hero
				let hero = TriggerHelper.SpawnUnits(sites[0],pickRandom(hero_templates),1,p);
	...
	for (let u of data.added)
	{
		...
		if (id != null)
		{
			if (id.classesList.indexOf("Champion") >= 0)
			{
				...
				{
					//pick patrol sites
					let sites = [pickRandom(structs),pickRandom(structs),pickRandom(structs)];
