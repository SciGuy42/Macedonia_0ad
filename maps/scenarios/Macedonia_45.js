...
Trigger.prototype.OwnershipChangedAction = function(data)
{
	...
	if (this.elephantAmbushTriggered == true && this.elephantAmbushEnded == false && data.from == 6 && data.to == -1)
	{
		const id = Engine.QueryInterface(data.entity, IID_Identity);
		// warn(uneval(id));
	...
	if (this.rebelFortressAttackStarted == false && data.from == 7 && data.to == -1)
	{
	...
	if ((data.from == 2 || data.from == 3 || data.from == 5) && (data.to == -1 || data.to == 1))
	{
		const id = Engine.QueryInterface(data.entity, IID_Identity);
		// warn(uneval(id));
		if (id != null && id.classesList.includes("Structure") && Math.random() < 0.25)
		{
...
Trigger.prototype.IntervalSpawnTraders = function(data)
{
	for (const p of [3, 4, 5, 6, 7])
	{
		...
		if (cmpPlayer.GetState() == "active")
		{
			...
			// spawn trader
			if (traders.length < 9 && markets.length > 0 && markets_others.length > 0)
			{
			...
			{
				const cmpUnitAI = Engine.QueryInterface(u, IID_UnitAI);
				if (cmpUnitAI && cmpUnitAI.IsIdle() && markets.length > 0 && markets_others.length > 0)
				{
					const spawn_market = pickRandom(markets);
					const target_market = pickRandom(markets_others);
...
Trigger.prototype.RangeActionGreekColony = function(data)
{
	if (this.greekColonyEvent == false)
	{
		...
		if (sites.length > 0)
		{
			...
			data.p = 7;
			data.site = spawn_site;
			data.templates = ["units/pers/champion_infantry", "units/pers/champion_elephant", "units/pers/arstibara", "units/pers/infantry_javelineer_e", "units/pers/infantry_archer_e", "units/pers/infantry_spearman_e", "units/pers/kardakes_hoplite", "units/pers/kardakes_skirmisher"];
			data.size = 12;
			data.target_class = "Unit";
			data.target_player = 1;
...
{
	this.archerTrainingAvailable = !this.archerTrainingAvailable;
};
