...
Trigger.prototype.OwnershipChangedAction = function(data)
{
	...
	if (data.from == 0 && data.to == 1)
	{
		...
		else if (data.entity == 6839) // siege shop
		{
			...
			{
				const spawn_site = 6404; // alexander
...
Trigger.prototype.PatrolOrder = function(units, p)
{
	...
	const patrolTargetPool = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Structure").filter(TriggerHelper.IsInWorld);
	...
	{
		const target_k = Math.floor(Math.random() * patrolTargetPool.length);
...
Trigger.prototype.SpawnSquad = function(data)
{
	...
	const ccs = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Outpost").filter(TriggerHelper.IsInWorld);
	const site_j = pickRandom(ccs);

	const pers_inf_templates = ["units/pers/arstibara", "units/pers/champion_infantry", "units/pers/kardakes_hoplite", "units/pers/kardakes_skirmisher", "units/pers/infantry_archer_b"];

	const pers_ele_templates = TriggerHelper.GetTemplateNamesByClasses("Champion+Elephant+!Hero", "pers", undefined, undefined, true);

	const squad_size = 7;
	const ele_prob = 0.35; // probability of a squad having an elephant
	...
	{
		const unit_i = TriggerHelper.SpawnUnits(site_j, pickRandom(pers_inf_templates), 1, p);
		units.push(unit_i[0]);
	}
	...
	{
		const unit_i = TriggerHelper.SpawnUnits(site_j, pickRandom(pers_ele_templates), 1, p);
		units.push(unit_i[0]);
	}
	...
	// send to attack
	// find target
	const target = this.FindClosestTarget(units[0], 6, siegeTargetClass);
...
Trigger.prototype.SpawnCavalrySquad = function(data)
{
	...
	const pers_cav_templates = ["units/pers/cavalry_javelineer_a", "units/pers/cavalry_spearman_a", "units/pers/cavalry_axeman_a", "units/pers/champion_cavalry", "units/pers/champion_cavalry_archer"];

	const pers_ele_templates = TriggerHelper.GetTemplateNamesByClasses("Champion+Elephant+!Hero", "pers", undefined, undefined, true);

	const squad_size = 7;
	const ele_prob = 0.5; // probability of a squad having an elephant
	...
	for (let i = 0; i < squad_size; i++)
	{
		const unit_i = TriggerHelper.SpawnUnits(site_j, pickRandom(pers_cav_templates), 1, p);
		units.push(unit_i[0]);
	}
...
Trigger.prototype.CheckHolyGuardBattleEnd = function(data)
{
	...
	{
		const units_p = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(7), "Unit").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.SpawnFieldAttack = function(data)
{
	...
	const num_squads_inf = 12;
	const num_squads_inf_wave_2 = 8;
	const num_squads_cav = 5;
	const ele_prob = 0.5; // probability of a squad having an elephant
...
{
	// spawn unit
	const triggerPoint = pickRandom(this.GetTriggerPoints(triggerPointsSpy));
	const owner = 6;
	const u = TriggerHelper.SpawnUnits(triggerPoint, "units/kush/cavalry_javelineer_a", 1, owner);

	// give it order to walk away
	const target = this.FindClosestTarget(u[0], 6, "Structure");
...
Trigger.prototype.SeizePlayerTroops = function(data)
{
	...
	const units = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(1), "Unit").filter(TriggerHelper.IsInWorld);
	...
	for (const u of units)
	{
		if (u != alex)
		{
			...
			const target = this.FindClosestTarget(u, 6, "Structure");
...
Trigger.prototype.SpawnPyramidWarrior = function(data)
{
	...
	const unit_list_length = this.kush_inf_ordered.length;
...
Trigger.prototype.GarrisonEntities = function(data)
{
	...
	for (const p of [0, 2, 4, 5, 6])
	{
		...
		for (const e of towers_p)
		{
			...
			if (p == 0)
			{
				const archers_e = TriggerHelper.SpawnUnits(e, "units/kush/champion_infantry_amun", 3, p);
		...
		{
			// spawn the garrison inside the tower
			const archers_e = TriggerHelper.SpawnUnits(c, "units/kush/champion_infantry_amun", 1, p);
		...
		if (p == 6 || p == 4)
		{
			...
			{
				// spawn the garrison inside the tower
				const archers_e = TriggerHelper.SpawnUnits(c, "units/kush/infantry_archer_a", 1, p);
		...
		if (p == 5)
		{
			const embs_p = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Embassy").filter(TriggerHelper.IsInWorld);

			for (const c of embs_p)
			{
				// spawn the garrison inside the tower
				const archers_e = TriggerHelper.SpawnUnits(c, "units/kush/champion_infantry_amun", 5, p);
...
Trigger.prototype.RangeActionTriggerAttack = function(data)
{
	...
	if (this.questCaravanAttack == false && this.questCaravansGiven == true)
	{
		...
		if (num_caravans >= 1)
		{
			...
			{
				// decide what to spawn
				const unit_type = pickRandom(["units/pers/cavalry_spearman_e", "units/pers/cavalry_spearman_b", "units/pers/cavalry_spearman_e", "units/pers/cavalry_axeman_a", "units/pers/cavalry_javelineer_b", "units/pers/cavalry_archer_b"]);
				const spawned_units = TriggerHelper.SpawnUnits(spawn_site, unit_type, 1, owner);
...
Trigger.prototype.SpawnInitialPatrol = function(data)
{
	...
	// targets A
	const targets_A = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "MercenaryCamp").filter(TriggerHelper.IsInWorld);

	const targets_B = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Outpost").filter(TriggerHelper.IsInWorld);
	...
	for (let j = 0; j < num_patrols; j++)
	{
		...
		// send to patrol
		const patrol_sites_j = [pickRandom(targets_A), pickRandom(targets_B), pickRandom(targets_A), pickRandom(targets_B)];
...
Trigger.prototype.QuestFieldBattleReward = function(data)
{
	...
	const spawn_sites = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(6), "MercenaryCamp").filter(TriggerHelper.IsInWorld);
...
{
	// lose siege towers
	const units_siege = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(1), "SiegeTower").filter(TriggerHelper.IsInWorld);
	for (const e of units_siege)
	{
		var cmpOwnership = Engine.QueryInterface(e, IID_Ownership);
		cmpOwnership.SetOwner(5);
	}
...
{
	const healers = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(1), "Healer").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.SpawnBanditGuards = function(data)
{
	const size = 20;
	const p = 2;
	const templates = ["units/kush/infantry_spearman_e", "units/kush/infantry_swordsman_a", "units/kush/infantry_javelineer_merc_a", "units/kush/infantry_spearman_b"];

	const patrol_sites = this.GetTriggerPoints(triggerPointsBanditGuards);

	for (let i = 0; i < size; i++)
	{
		const spawn_site = pickRandom(patrol_sites);
...
{
	const healers = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(8), "Healer").filter(TriggerHelper.IsInWorld);

	const heroes = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(8), "Hero").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.BossHealersCheck = function(data)
{
	...
	{
		const cmpUnitAI = Engine.QueryInterface(h, IID_UnitAI);
...

{
	...
	cmpTrigger.kush_inf_ordered = ["units/kush/infantry_archer_b", "units/kush/infantry_spearman_b", "units/kush/infantry_archer_a", "units/kush/infantry_spearman_a", "units/kush/infantry_swordsman_b", "units/kush/infantry_archer_e", "units/kush/infantry_pikeman_b", "units/kush/infantry_swordsman_a", "units/kush/infantry_clubman_b", "units/kush/champion_infantry_archer", "units/kush/infantry_pikeman_a", "units/kush/cavalry_javelineer_merc_b", "units/kush/infantry_javelineer_merc_b", "units/kush/infantry_spearman_e", "units/kush/infantry_clubman_a", "units/kush/cavalry_javelineer_merc_a", "units/kush/infantry_pikeman_e", "units/kush/infantry_javelineer_merc_a", "units/kush/infantry_swordsman_e", "units/kush/cavalry_javelineer_merc_a", "units/kush/infantry_javelineer_merc_e", "units/kush/champion_infantry_archer", "units/kush/infantry_clubman_e", "units/kush/champion_infantry_amun", "units/kush/champion_cavalry", "units/kush/champion_infantry_apedemak", "units/kush/champion_elephant"];
