		...
		if (id)
		{
			...
			else if (id.classesList.indexOf("Dock") >= 0)
			{
				...
				if (this.numDocksCaptured == 1)
				{
					...
					const trader = TriggerHelper.SpawnUnits(data.entity, "units/athen/support_trader", 5, 1);
			...
			else if (id.classesList.indexOf("Arsenal") >= 0)
			{
				...
				const p = 1;
				const siege = TriggerHelper.SpawnUnits(data.entity, "units/mace/siege_lithobolos_packed", 4, p);
...
Trigger.prototype.GarrisonEntities = function(data)
{
	...
	for (const p of [0, 2, 3, 4, 5, 6])
	{
		...
		for (const e of towers_p)
		{
			...
			const archers_e = TriggerHelper.SpawnUnits(e, "units/athen/champion_ranged", g_size, p);
		...
		{
			// spawn the garrison inside the tower
			const archers_e = TriggerHelper.SpawnUnits(e, "units/athen/champion_ranged", 3, p);
		...
		// outpost tower
		const towers_o = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Outpost").filter(TriggerHelper.IsInWorld);

		for (const e of towers_o)
		{
			// spawn the garrison inside the tower
			const archers_e = TriggerHelper.SpawnUnits(e, "units/athen/champion_ranged", 1, p);
		...
		for (const e of forts_p)
		{
			...
			if (p == 0)
			{
				const archers_e = TriggerHelper.SpawnUnits(e, "units/gaul/champion_fanatic", fort_size, p);
...
Trigger.prototype.IdleUnitCheck = function(data)
{
	...
	for (const p of [2, 4, 6])
	{
		...
		const units_all = units_inf.concat(units_cav, units_siege);

		if (units_all.length > 0)
		{
			// warn("Found idle units.");

			const targets_A = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "CivilCentre").filter(TriggerHelper.IsInWorld);

			const targets_B = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "SentryTower").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.SpawnPatrolInterval = function(data)
{
	...
	const interval_secs = this.patrolSpawnTime;
...
Trigger.prototype.SpawnRepairCrew = function(data)
{
	...
	{
		const health_s = Engine.QueryInterface(s, IID_Health);
	...
	if (damaged_buildings.length > 0)
	{
		...
		const current_citizens = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Citizen").filter(TriggerHelper.IsInWorld);

		// warn("Found "+uneval(current_citizens.length)+" citizens");

		if (current_citizens.length < this.maxRepairmen)
		{
			const sites = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "CivilCentre").filter(TriggerHelper.IsInWorld);

			const site_j = pickRandom(sites);

			if (site_j)
			{
				const repair_crew = TriggerHelper.SpawnUnits(site_j, pickRandom(this.repairTemplates), 5, p);

				// warn("Sending repair men");

				for (const b of damaged_buildings)
				{
					ProcessCommand(p, {
						"type": "repair",
...
Trigger.prototype.SpawnCavalryDockAttack = function(data)
{
	...
	const docks = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(target_player), "Dock").filter(TriggerHelper.IsInWorld);

	if (docks.length > 2)
	{
		const site = pickRandom(this.GetTriggerPoints(triggerPointScythianCavalryDock));
		...
		for (let i = 0; i < wave_size; i++)
		{
			...
			const unit_i = TriggerHelper.SpawnUnits(site, pickRandom(this.scythianCavTemplates), 1, p);
		...
		// find target
		const target = this.FindClosestTarget(attackers[0], target_player, "Dock");
...
Trigger.prototype.SpawnCavalryWave = function(data)
{
	...
	{
		// get site
		const site_i = pickRandom(this.GetTriggerPoints(triggerPointScythianCavalry));

		const wave_size = this.scythianWaveSize;
		const attackers = [];

		for (let i = 0; i < wave_size; i++)
		{

			const unit_i = TriggerHelper.SpawnUnits(site_i, pickRandom(this.scythianCavTemplates), 1, p);
			attackers.push(unit_i[0]);
		...
		// find target
		const target = this.FindClosestTarget(attackers[0], 3, unitTargetClass);
		const target_pos = TriggerHelper.GetEntityPosition2D(target);
...
Trigger.prototype.SpawnPatrol = function(data)
{
	...
	if (units.length < this.maxPatrolNumber)
	{
		...
		{
			const unit_i = TriggerHelper.SpawnUnits(site_j, pickRandom(this.patrolTemplates), 1, p);
		...
		{
			// attack
			const target = this.FindClosestTarget(patrol_j[0], 3, unitTargetClass);
			const target_pos = TriggerHelper.GetEntityPosition2D(target);
...
Trigger.prototype.SpawnOlbianTrader = function(data)
{
	...
	const docks = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(e), "Trade+!Dock").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.VictoryAchieved = function(data)
{
	...
	const structures = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(3), "Structure").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.SpawnCavalrySiegeAttack = function(data)
{
	...
	const siege_units = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(3), "Siege").filter(TriggerHelper.IsInWorld);
	...
	{
		const cav_units = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(2), "Cavalry").filter(TriggerHelper.IsInWorld);

		if (cav_units.length < this.cavLimit)
		{

			// site
			const site_j = pickRandom(this.GetTriggerPoints("A"));

			// spawn and give orders
			for (let i = 0; i < this.cavSquadSize; i++)
			{
				const units_i = TriggerHelper.SpawnUnits(site_j, pickRandom(this.cavTemplates), 1, p);
	...
	for (const p of [2, 4, 5])
	{
		...
		for (const t of traders)
		{
			...
			if (goods_i.amount)
			{
				...
				{
					cargo_food += goods_i.amount.traderGain;
				}
				else if (goods_i.type == "wood")
				{
					cargo_wood += goods_i.amount.traderGain;
				}
				else if (goods_i.type == "stone")
				{
					cargo_stone += goods_i.amount.traderGain;
				}
				else if (goods_i.type == "metal")
				{
					cargo_metal += goods_i.amount.traderGain;
				}
			}
		}
...
Trigger.prototype.SpawnSlaveAttack = function(data)
{
	...
	{
		const unit_i = TriggerHelper.SpawnUnits(site, pickRandom(this.slaveTemplates), 1, p);
		attackers.push(unit_i[0]);
	}
...
Trigger.prototype.PointIncrement = function(data)
{
	...
	const sites = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Structure").filter(TriggerHelper.IsInWorld);
	...
	for (const u of units_to_spawn)
	{
		// pick random site
		const site_j = pickRandom(sites);

		// spawn unit
		const units_i = TriggerHelper.SpawnUnits(site_j, u, 1, p);
