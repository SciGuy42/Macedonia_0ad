...
Trigger.prototype.IdleUnitCheck = function(data)
{
	...
	for (const p of [5])
	{
		...
		const siege_units = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Siege").filter(TriggerHelper.IsInWorld);
		...
		// find targets
		const structs = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(1), "Structure").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.GarrisonEntities = function(data)
{
	for (const p of [1, 5, 6])
	{
		...
		{
			// spawn the garrison inside the tower
			const archers_e = TriggerHelper.SpawnUnits(c, "units/maur/champion_infantry_maceman", 10, p);
	...
	// garisson ships
	const biremes = [17425, 17426]; // 20
	const triremes = [17427]; // 30
	const trade_ships = [17428, 17429]; // 15

	for (const ship of biremes)
	{
...
Trigger.prototype.IntervalSpawnGuards = function(data)
{
	...
	if (units.length < pop_limit)
	{
		...
		for (const a of [2, 3])
		{
			const cmpPlayer_a = QueryPlayerIDInterface(a);
		...
		const ally = pickRandom(allies);

		// find patrol/spawn sites
		const patrol_sites = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(ally), "Structure").filter(TriggerHelper.IsInWorld);

		if (patrol_sites.length > 4)
		{

			const inf_templates = ["units/pers/kardakes_hoplite", "units/pers/kardakes_skirmisher", "units/pers/infantry_archer_e", "units/pers/champion_infantry"];
...
Trigger.prototype.SpawnInitialGuards = function(data)
{
	...
	for (let i = 0; i < 100; i++)
	{
		...
		if (patrol_sites.length > 4)
		{
			...
			const inf_templates = ["units/pers/kardakes_hoplite", "units/pers/kardakes_skirmisher", "units/pers/infantry_archer_e", "units/pers/champion_infantry", "units/pers/infantry_spearman_e"];
