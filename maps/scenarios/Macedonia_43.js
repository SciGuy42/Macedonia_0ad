...
Trigger.prototype.StructureDecayCheck = function(data)
{
	...
	for (const p of [1, 5])
	{
		const slaves = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Slave").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.GarrisonEntities = function(data)
{
	for (const p of [2])
	{
		...
		{
			// spawn the garrison inside the tower
			const archers_e = TriggerHelper.SpawnUnits(c, "units/pers/arstibara", 20, p);
...
Trigger.prototype.VictoryCheck = function(data)
{
	...
	const units = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(0), "Soldier").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.OwnershipChangedAction = function(data)
{
	...
	{
		const soldiers = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(5), "Soldier").filter(TriggerHelper.IsInWorld);

		const barracks = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(5), "Barracks").filter(TriggerHelper.IsInWorld);
	...
	if (data.from == 6 && data.to == -1)
	{
		...
		const soldiers = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(6), "Unit").filter(TriggerHelper.IsInWorld);

		for (const u of soldiers)
		{
			const cmpUnitAI = Engine.QueryInterface(u, IID_UnitAI);
			if (cmpUnitAI && cmpUnitAI.order && cmpUnitAI.order.type && cmpUnitAI.order.type == "Patrol")
			{
				// send to attack
		...
		const id = Engine.QueryInterface(data.entity, IID_Identity);
		if (id != null && id.classesList.includes("Infantry") && Math.random() < 0.25)
		{
			// spawn cavalry man and send him to attack
			const sites = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(4), "House").filter(TriggerHelper.IsInWorld);

			const templates = ["units/pers/cavalry_javelineer_e", "units/pers/cavalry_spearman_e", "units/pers/cavalry_axeman_e"];

			const target_pos = TriggerHelper.GetEntityPosition2D(data.entity);

			const unit_i = TriggerHelper.SpawnUnits(pickRandom(sites), pickRandom(templates), 1, 6);
...
Trigger.prototype.IntervalSpawnTraders = function(data)
{
	...
	if (traders.length < 6)
	{
		...
		const markets = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(4), "Trade").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.QuestTempleComplete = function(data)
{
	...
	const unit_i = TriggerHelper.SpawnUnits(site, "units/pers/support_healer_e", 7, 1);
...
Trigger.prototype.RangeActionTemple = function(data)
{
	...
	else if (this.questTempleComplete == false && this.questTempleRelicTaken == true)
	{
		// complete quest
		this.QuestTempleComplete();
	}
	...
	if (data.cmd.type == "dialog-answer")
	{
		...
		if (this.dialogState == "mercs")
		{
			if (data.cmd.answer == "button1")
			{
				...
				// get the units
				const site = this.GetTriggerPoints(triggerPointsMercs)[0];

				const mercs = TriggerHelper.SpawnUnits(site, "units/pers/champion_cavalry_archer", 10, 1);
		...
		else if (this.dialogState == "elephant_traders")
		{
			if (data.cmd.answer == "button1")
			{
				...
				// get the units
				const site = this.GetTriggerPoints(triggerPointsElephantTraders)[0];

				const mercs = TriggerHelper.SpawnUnits(site, "units/pers/champion_elephant", 3, 1);
				const support_elephant = TriggerHelper.SpawnUnits(site, "units/maur/support_elephant", 1, 1);
...
{
	this.elephantsAvailable = !this.elephantsAvailable;
};
...
Trigger.prototype.RangeActionMercs = function(data)
{
	...
	if (this.mercsAvailable == true && data.added.length > 0)
	{
		...
		if (resources.metal >= total_cost_metal)
		{
			...
			const offer_text = "You encounter a small camp used by local mercenaries.  A number of horse-riding archers are available for hire but it will cost us 1500 metal. ";
