...
Trigger.prototype.GarrisonEntities = function(data)
{
	for (let p of [2])
	{
		...
		{
			//spawn the garrison inside the tower
			let archers_e = TriggerHelper.SpawnUnits(c, "units/pers_arstibara",20,p);
	...
	if (data.from == 6 && data.to == -1)
	{
		...
		if (id != null && id.classesList.indexOf("Infantry") >= 0)
		{
			if (Math.random() < 0.25)
			{
				...
				let templates = ["units/pers_cavalry_javelinist_e","units/pers_cavalry_spearman_e","units/pers_cavalry_swordsman_e"];
	...
	if (data.cmd.type == "dialog-answer")
	{
		...
		if (this.dialogState == "mercs")
		{
				if (data.cmd.answer == "button1")
				{
					...
					let mercs = TriggerHelper.SpawnUnits(site,"units/pers_champion_cavalry_archer",10,1);
		...
		else if (this.dialogState == "elephant_traders")
		{
			if (data.cmd.answer == "button1")
			{
				...
				let mercs = TriggerHelper.SpawnUnits(site,"units/pers_champion_elephant",3,1);
				let support_elephant = TriggerHelper.SpawnUnits(site,"units/maur_support_elephant",1,1);
