...
Trigger.prototype.PatrolOrder = function(units, p)
{
	...
	const gates = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(2), "Military").filter(TriggerHelper.IsInWorld);
	const docks = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(2), "Dock").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.SpawnAlliedInvasionAttack = function(data)
{
	...
	const ungarrison_point = pickRandom(this.GetTriggerPoints(target_point));
...
Trigger.prototype.PersianCavalryAttack = function(data)
{
	...
	// find target
	const target = this.FindClosestTarget(attackers[0], 1, "Structure+!Lighthouse");
...
Trigger.prototype.SpawnShipPatrolPeriodic = function(data)
{
	...
	const warships = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Warship").filter(TriggerHelper.IsInWorld);
	...
	// pick ship template
	const pers_ships_types = TriggerHelper.GetTemplateNamesByClasses("Warship", "pers", undefined, undefined, true);

	// spawn ship
	const ship_spawned = TriggerHelper.SpawnUnits(spawn_site, pickRandom(pers_ships_types), 1, p);
	...
	// make patrol
	const sites = this.GetTriggerPoints(triggerPointShipPatrol);
	// warn(uneval(sites));
	const patrol_sites = [pickRandom(sites), pickRandom(sites), pickRandom(sites), pickRandom(sites), pickRandom(sites), pickRandom(sites)];
...
Trigger.prototype.SpawnElephantPatrolPeriodic = function(data)
{
	...
	const elephants = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p), "Elephant").filter(TriggerHelper.IsInWorld);
	...
	// spawn elephant
	const unit_i = TriggerHelper.SpawnUnits(pickRandom(ccs), "units/pers/champion_elephant", 1, p);
...
Trigger.prototype.SpawnPatrolPeriodic = function(data)
{
	...
	const currentPop = QueryPlayerIDInterface(p).GetPopulationCount();
	if (currentPop > 290)
		return;
	...
	for (let j = 0; j < num_patrols; j++)
	{
		...
		for (let i = 0; i < 3; i++)
		{
			const unit_i = TriggerHelper.SpawnUnits(site_j, "units/cart/champion_infantry", 1, p);
			units.push(unit_i[0]);
		}
		...
		for (let i = 0; i < 2; i++)
		{
			const unit_i = TriggerHelper.SpawnUnits(site_j, "units/cart/infantry_archer_e", 1, p);
			units.push(unit_i[0]);
		}
...
Trigger.prototype.IdleUnitCheck = function(data)
{
	...
	for (const player of [4])
	{
		const units_inf = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(player), "Elephant").filter(TriggerHelper.IsInWorld);

		// make patrol if idle
		for (const unit of units_inf)
		{
			const cmpUnitAI = Engine.QueryInterface(unit, IID_UnitAI);
			if (cmpUnitAI && cmpUnitAI.IsIdle())
				this.PatrolOrder([unit], player);
		}
	}
};
