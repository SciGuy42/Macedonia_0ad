...
Trigger.prototype.GarrisonEntities = function(data)
{
	...
	for (const p of [4, 5, 6])
	{
		...
		for (const e of towers_p)
		{
			...
			const archers_e = TriggerHelper.SpawnUnits(e, "units/pers/infantry_archer_e", 5, owner);
		...
		for (const e of forts_p)
		{
			...
			const archers_e = TriggerHelper.SpawnUnits(e, "units/pers/infantry_archer_e", fort_size, owner);
	...
	for (const p of [2, 4, 5])
	{
		...
		if (markets_p.length > 0)
		{
			...
			if (traders_p.length < 12)
			{
				...
				let markets_others = [];
				const trading_partners = [2, 4, 5, 6];
				for (const p2 of trading_partners)
				{
					if (p2 != p)
					{
						const markets_p2 = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(p2), "Trade+!Dock").filter(TriggerHelper.IsInWorld);
...
Trigger.prototype.RangeActionWorkshop = function(data)
{
	if (this.workshopOffer == true)
	{
		...
		const siege = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(1), "Siege").filter(TriggerHelper.IsInWorld);

		const cost = this.workshopBaseCost * Math.pow(this.workshopSizeFactor, siege.length);
		...
		if (resources.food > cost && resources.metal > Math.round(cost * 0.5))
		{
			...
			const names = ["Siege Ram", "Catapult", "Bolt Shooter"];
...
Trigger.prototype.SpawnInfiltrators = function(data)
{
	...
	const units = TriggerHelper.MatchEntitiesByClass(TriggerHelper.GetEntitiesByPlayer(7), "Infantry").filter(TriggerHelper.IsInWorld);
	...
	// rebel templates
	const templates = ["units/pers/infantry_archer_e", "units/pers/infantry_spearman_e", "units/pers/infantry_javelineer_e", "units/pers/infantry_spearman_a", "units/pers/kardakes_hoplite"];
